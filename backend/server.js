...
app.post("/products", async (req, res) => {
  try {
    ...
    const sql =
      "INSERT INTO products (shop_id, product_name, category, price, stock) VALUES (?, ?, ?, ?, ?)";
    const [result] = await conn.execute(sql, [
      shop_id,
      productName,
      category,
...
app.put("/products/:id", async (req, res) => {
  try {
    ...
    const sql =
      "UPDATE products SET product_name = ?, category = ?, price = ?, stock = ? WHERE id = ? AND shop_id = ?";
    const [result] = await conn.execute(sql, [
      productName,
      category,
      price,
...
app.post("/payments", async (req, res) => {
  try {
    ...
    const sql =
      "INSERT INTO payments (shop_id, customer_name, mobile_no, amount, payment_type) VALUES (?, ?, ?, ?, ?)";
    const [result] = await conn.execute(sql, [
      shop_id,
      customerName,
      mobileNo,
...
app.put("/payments/:id", async (req, res) => {
  try {
    ...
    const sql =
      "UPDATE payments SET customer_name = ?, mobile_no = ?, amount = ?, payment_type = ? WHERE id = ? AND shop_id = ?";
    const [result] = await conn.execute(sql, [
      customerName,
      mobileNo,
      amount,
...
app.get("/api/shop/:id", async (req, res) => {
  try {
    ...
    const sql =
      "SELECT id, shop_name, owner_name, mobile, email FROM shops WHERE id = ?";
    const [results] = await conn.execute(sql, [shopId]);
...
app.put("/api/shop/:id", async (req, res) => {
  try {
    ...
    const sql =
      "UPDATE shops SET shop_name = ?, owner_name = ?, mobile = ?, email = ? WHERE id = ?";
